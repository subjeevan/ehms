...
export function normalizePatient(patient) {
  return {
    ...
    insuranceDetail: {
      provider:
          patient?.insuranceDetail?.provider ||
          "",
...
export function validatePatient(values) {
  ...
  if (!values.fullName?.trim()) {
    ...
    errors.fullName =
      ...
      values.fullName.trim().length > 150
  ) {
    errors.fullName =
  ...
  if (!values.dateOfBirth) {
    errors.dateOfBirth =
      ...
      !datePattern.test(values.dateOfBirth)
  ) {
    errors.dateOfBirth =
        "Date must contain a four-digit year";
  } else if (
      !isValidCalendarDate(values.dateOfBirth)
  ) {
    errors.dateOfBirth =
  ...
  if (!contactNumber) {
    errors.contactNumber =
      ...
      !contactPattern.test(contactNumber)
  ) {
    errors.contactNumber =
        "Contact number must contain between 8 and 25 digits";
  }

  if (!values.address?.trim()) {
    errors.address = "Address is required";
  } else if (
      values.address.trim().length > 300
  ) {
    errors.address =
  ...
  if (
    ...
    if (!insurance.provider?.trim()) {
      errors[
        ...
        insurance.provider.trim().length >
        120
    ) {
      errors[
          "insuranceDetail.provider"
          ] =
          "Provider must not exceed 120 characters";
    }
...
export function toPatientPayload(values) {
  const payload = {
    ...
    /*
     * This must be null for
     * GENERAL and PAYING patients.
     */
  ...
  if (
    ...
    payload.insuranceDetail = {
      provider:
          values.insuranceDetail.provider.trim(),

      policyNumber:
          values.insuranceDetail.policyNumber.trim(),
      ...
      expiryDate:
      values.insuranceDetail.expiryDate,
    };
  }
