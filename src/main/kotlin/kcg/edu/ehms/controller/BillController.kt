...
class BillController(private val service: BillService) {
    ...
    fun listForPatient(@PathVariable patientId: Long): ResponseEntity<List<BillResponse>> =
        ResponseEntity.ok(service.listForPatient(patientId))

    @GetMapping("/visits/{visitId}/bills")
    @PreAuthorize("hasAnyRole('ADMIN','USER')")
    fun listForVisit(@PathVariable visitId: Long): ResponseEntity<List<BillResponse>> =
        ResponseEntity.ok(service.listForVisit(visitId))

    @PostMapping("/patients/{patientId}/bills")
    @PreAuthorize("hasRole('ADMIN')")
    fun createForLatestVisit(
    ...
    fun createForVisit(
        ...
        ResponseEntity.status(HttpStatus.CREATED).body(service.createForVisit(visitId, request))
